...
class MY_Controller extends MX_Controller {
	...
	private function _setup()
	{
		...
		if($this->mModule == 'ketoan'){
			if(!is_accountant()){
				redirect('../errors/page_missing');
			}
			$config_ketoan = $this->config->item('ci_bootstrap_ketoan');
			$config['menu'] = $config_ketoan['menu'];
		}
		else if($this->mModule == 'nhansu'){
			if(!is_hr()){
				redirect('../errors/page_missing');
			}
			$config_nhansu = $this->config->item('ci_bootstrap_nhansu');
			$config['menu'] = $config_nhansu['menu'];
		}
		...
		if ( $this->ion_auth->logged_in() )
		{
			...
			if ( !empty($this->mUser) )
			{
				...
				$this->db->update(	
					...
					array('id' => $this->mUser->id)
				);

				if(empty(get_user_stores())){
					echo '<pre>';
					print_r('Tài khoản chưa được chọn Chi nhánh. Vui lòng liên hệ quản trị viên');
					echo '</pre>';
				...
				if(empty($this->session->userdata('store_id'))){
					$this->session->set_userdata('store_id', get_user_stores()[0]->id);
				}
			...
			if($this->mUser->birthday == '0000-00-00' && $this->mUser->phone_contact == '' && $this->mCtrler != 'panel' ){
	...
	protected function verify_login($redirect_url = NULL)
	{
		if ( !$this->ion_auth->logged_in() )
		{
			if ( $redirect_url==NULL ){
				...
				$redirect_url = $this->mConfig['login_url'].'?redirect='.$redirect_url;
			}
			redirect($redirect_url);
		}
	}
